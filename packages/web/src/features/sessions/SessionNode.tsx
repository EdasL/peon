...
const PROGRESS_TRANSITION = "width 0.7s ease-out, box-shadow 0.7s ease-out"
...
function arePropsEqual(
  prev: SessionNodeProps,
  next: SessionNodeProps,
): boolean {
  return (
...
export const SessionNode = memo(function SessionNode({
  ...
  const colors =
    pct >= 80 ? COLORS_CRITICAL : pct >= 50 ? COLORS_WARNING : COLORS_NORMAL
  ...
  const hasRenameAction = Boolean(
    canRenameDelete && onStartRename && !isRenaming,
  )
  ...
  const handleAbortFromMenu = useCallback(
    (e: React.MouseEvent) => {
      handleAbortClick(e)
      setActionsOpen(false)
    },
    ...
    isCron || isCronRun
      ? running
        ...
        : isCron
          ...
          : "RUN"
      : granularStatus
        ? getStatusBadgeText(granularStatus)
        : running
          ? "WORKING"
          : "IDLE"
  ...
  return (
    <div
      className={cn(
        ...
        isActive &&
          "border-l-[3px] border-l-primary bg-primary/5 shadow-[inset_0_0_12px_rgba(232,168,56,0.06)]",
        isUnread && !isActive && "bg-green-500/5",
          ...
          <SessionInfoPanel session={node.session} running={running}>
            ...
            >
              {isCron && (
                <Timer
                  size={11}
                  className="text-purple-400 mr-1 inline shrink-0"
                  aria-label="Cron job"
                />
              )}
              {isCronRun && (
                <CornerDownRight
                  size={10}
                  className="text-purple-400/60 mr-1 inline shrink-0"
                  aria-label="Cron run"
                />
              )}
              {label}
