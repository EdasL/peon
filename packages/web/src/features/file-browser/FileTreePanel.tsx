...
import { PanelLeftClose, PanelLeftOpen, RefreshCw, Pencil, Trash2, RotateCcw, X, AlertTriangle } from 'lucide-react';
...
const WIDTH_STORAGE_KEY = 'peon-file-tree-width';
const COLLAPSED_STORAGE_KEY = 'peon-file-tree-collapsed';
...
/** Inline confirm dialog — avoids external component dependency. */
  ...
  return (
    <>
      ...
      <div
        role="alertdialog"
        aria-modal="true"
        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-card border border-border p-6 z-50 min-w-[300px] max-w-md rounded-lg shadow-xl"
      >
        ...
        <div className="flex justify-end gap-2">
          <button onClick={onCancel} className="px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground border border-border rounded hover:border-muted-foreground">
            Cancel
          </button>
          <button onClick={onConfirm} className="px-3 py-1.5 text-xs font-bold bg-destructive text-destructive-foreground rounded hover:bg-destructive/90">
            {confirmLabel}
          </button>
        </div>
      </div>
...
export function FileTreePanel({
  ...
  useEffect(() => {
    ...
    const maxX = Math.max(MENU_VIEWPORT_PADDING, window.innerWidth - menuWidth - MENU_VIEWPORT_PADDING);
    const maxY = Math.max(MENU_VIEWPORT_PADDING, window.innerHeight - menuHeight - MENU_VIEWPORT_PADDING);
  ...
  const rpcFileOp = useCallback(async (method: string, params: Record<string, unknown>): Promise<FileOpResult> => {
    const result = await rpc(method, params) as FileOpResult;
    if (!result.ok) {
    ...
    try {
      if (targetDirPath === '.trash' && !sourcePath.startsWith('.trash/')) {
        if (workspaceInfo?.isCustomWorkspace) {
          const result = await rpcFileOp('files.move', { sourcePath, targetDirPath: '.trash' });
          refresh();
        ...
        const result = await rpcFileOp('files.trash', { path: sourcePath });
      ...
      const result = await rpcFileOp('files.move', { sourcePath, targetDirPath });
      refresh();
      onRemapOpenPaths?.(result.from, result.to);
      selectFile(result.to);
  ...
  }, [onCloseOpenPaths, onRemapOpenPaths, rpcFileOp, refresh, selectFile, showToast, workspaceInfo]);
  ...
  const commitRename = useCallback(async () => {
    ...
    try {
      const result = await rpcFileOp('files.rename', { path: renameTargetPath, newName: nextName });
      cancelRename();
      refresh();
      onRemapOpenPaths?.(result.from, result.to);
      selectFile(result.to);
  ...
  }, [cancelRename, onRemapOpenPaths, rpcFileOp, refresh, renameTargetPath, renameValue, selectFile, showToast]);
  ...
  const moveToTrash = useCallback(async (entry: TreeEntry) => {
    ...
    try {
      const result = await rpcFileOp('files.trash', { path: entry.path });
      onCloseOpenPaths?.(result.from);
      refresh();
      setContextMenu(null);
      showToast(
  ...
  }, [onCloseOpenPaths, rpcFileOp, refresh, showToast, workspaceInfo]);
  ...
  }, [onCloseOpenPaths, rpcFileOp, refresh, showToast]);

  const restoreEntry = useCallback(async (entryPath: string) => {
    try {
      const result = await rpcFileOp('files.restore', { path: entryPath });
      refresh();
      onRemapOpenPaths?.(result.from, result.to);
      selectFile(result.to);
  ...
  }, [onRemapOpenPaths, rpcFileOp, refresh, selectFile, showToast]);
