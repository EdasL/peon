...
export const restartProject = (id: string) =>
  request<{ status: string }>(`/api/projects/${id}/restart`, {
    method: "POST",
  });
...
export const getApiKeys = () =>
  request<{ keys: ApiKeyInfo[]; oauthConnections: OAuthConnection[] }>(
    "/api/keys",
  );
export const addApiKey = (data: {
  ...
  request<{ key: ApiKeyInfo }>("/api/keys", {
    method: "POST",
    body: JSON.stringify(data),
  });
...
export const initClaudeOAuth = () =>
  request<{ authUrl: string }>("/api/auth/claude-oauth/web-init", {
    method: "POST",
  });
...
export async function fetchTasks(projectId: string): Promise<ClaudeTask[]> {
  const { tasks } = await request<{ tasks: ClaudeTask[] }>(
...
export async function createTask(
  ...
  const { task } = await request<{ task: ClaudeTask }>(
    `/api/projects/${projectId}/tasks`,
    {
      method: "POST",
      body: JSON.stringify(data),
...
export async function deleteTask(projectId: string, id: string): Promise<void> {
  await request<void>(`/api/projects/${projectId}/tasks/${id}`, {
    method: "DELETE",
  });
}
...
export interface ContentBlock {
  ...
  tool_use_id?: string;
  /** Client-only: true while the tool call is still in progress during streaming */
  _loading?: boolean;
}
